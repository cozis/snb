...
void GuiWindowFileDialog(GuiWindowFileDialogState *state)
{
    if (state->windowActive)
    {
        ...
        if (GuiButton((Rectangle){ state->windowBounds.x + state->windowBounds.width - 48, state->windowBounds.y + 12, 40, 24 }, "< .."))
        {
        ...
        if (GuiTextBox((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + 12, state->windowBounds.width - 48 - 16, 24 }, state->dirPathText, 1024, state->dirPathEditMode))
        {
...
# if defined(USE_CUSTOM_LISTVIEW_FILEINFO)
        state->filesListActive = GuiListViewFiles((Rectangle){ state->position.x + 8, state->position.y + 24 + 20, state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68 + 24 }, fileInfo, state->dirFiles.count, &state->itemFocused, &state->filesListScrollIndex, state->filesListActive);
# else
        GuiListViewEx((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + 24 + 20, state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68 + 24 }, 
