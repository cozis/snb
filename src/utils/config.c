...
error(CfgError *err, const char *fmt, ...)
{
    ...
    va_list vargs;
    va_start(vargs, fmt);
    vsnprintf(err->msg, CFG_MAX_ERR+1, fmt, vargs);
    va_end(vargs);
    return -1;
}
...
{
    bool location = (err->row > 0 && err->col > 0);
...
read_file(const char *filename, int *size, char *err)
{
    ...
    if (!file) {
        snprintf(err, CFG_MAX_ERR + 1, "failed to open '%s'", filename);
        return NULL;
    }
    ...
    if (bytes_read != size_) {
        free(src);
        snprintf(err, CFG_MAX_ERR + 1, "failed to read '%s'", filename);
        return NULL;
    }
...
static const void *
get_val(Cfg cfg, const char *key, const void *default_, CfgValType type)
{
...
const char *
cfg_get_str(Cfg cfg, const char *key, const char *default_)
{
